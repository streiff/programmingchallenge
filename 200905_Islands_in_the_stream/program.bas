...
4500 ' Finds islands and remarks them as part of the
4501 ' surrounding landmass
4502 ' relies on: LINES$ - map data that has been marked
4503 '            ROWNUM - number of rows in the map
4504 '            COLNUM - number of columns in the map
4510 FOR X = 0 TO ROWNUM - 1
4520 FOR Y = 1 TO COLNUM
4530 IF FNCHARAT$(LINES$(X), Y) <> "-" THEN GOTO 5000
4535 IF FNCHARAT$(LINES$(X + 1), Y) = "-" THEN GOTO 5000
4536 TARGET$ = FNCHARAT$(LINES$(X + 1), Y)
4560 FOR Z = X TO 0 STEP -1
4570 IF FNCHARAT$(LINES$(Z), Y) = "-" THEN GOTO 4900
4580 SOURCE$ = FNCHARAT$(LINES$(Z), Y)
4585 IF SOURCE$ = TARGET$ THEN GOTO 4890
4589 MID$(LINES$(X + 1), Y) = SOURCE$
4590 GOSUB 2500
4890 Z = 0
4900 NEXT
5000 NEXT
